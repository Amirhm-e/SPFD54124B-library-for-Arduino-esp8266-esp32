...
bool mSPIClass::pins(int8_t sck, int8_t miso, int8_t mosi, int8_t ss)
{
    if (sck == 6 &&
        miso == 7 &&
        mosi == 8 &&
...
void mSPIClass::setHwCs(bool use) {
    switch (pinSet) {
    case SPI_PINS_HSPI:
        if (use) {
...
void mSPIClass::beginTransaction(mSPISettings settings) {
...
void mSPIClass::endTransaction() {
}

void mSPIClass::setDataMode(uint8_t dataMode) {
...
void mSPIClass::setBitOrder(uint8_t bitOrder) {
...
void mSPIClass::setFrequency(uint32_t freq) {
...
void mSPIClass::setClockDivider(uint32_t clockDiv) {
...
inline void mSPIClass::setDataBits(uint16_t bits) {
    const uint32_t mask = ~((SPIMMOSI << SPILMOSI) | (SPIMMISO << SPILMISO));
    bits--;
    SPI1U1 = ((SPI1U1 & mask) | ((bits << SPILMOSI) | (bits << SPILMISO)));
}

uint8_t mSPIClass::transfer(uint8_t data) {
    while(SPI1CMD & SPIBUSY) {}
    // reset to 8Bit mode
    setDataBits(8);
    SPI1W0 = data;
...
uint16_t mSPIClass::transfer16(uint16_t data) {
    union {
            uint16_t val;
            struct {
                    uint8_t lsb;
...
void mSPIClass::transfer(void *buf, uint16_t count) {
...
void mSPIClass::write(uint8_t data) {
    while(SPI1CMD & SPIBUSY) {}
    // reset to 8Bit mode
    setDataBits(8);
    SPI1W0 = data;
...
void mSPIClass::write16(uint16_t data) {
    write16(data, !(SPI1C & (SPICWBO | SPICRBO)));
}

void mSPIClass::write9(uint16_t data) {
    while(SPI1CMD & SPIBUSY) {}
...
void mSPIClass::write16(uint16_t data, bool msb) {
    while(SPI1CMD & SPIBUSY) {}
...
void mSPIClass::write32(uint32_t data) {
    write32(data, !(SPI1C & (SPICWBO | SPICRBO)));
}

void mSPIClass::write32(uint32_t data, bool msb) {
    while(SPI1CMD & SPIBUSY) {}
    // Set to 32Bits transfer
    setDataBits(32);
    if(msb) {
 ...
 * @param size uint32_t
 */
void mSPIClass::writeBytes(const uint8_t * data, uint32_t size) {
    while(size) {
        if(size > 64) {
            writeBytes_(data, 64);
            size -= 64;
...
void mSPIClass::writeBytes_(const uint8_t * data, uint8_t size) {
    while(SPI1CMD & SPIBUSY) {}
 ...
 */
void mSPIClass::writePattern(const uint8_t * data, uint8_t size, uint32_t repeat) {
 ...
 * @param size uint32_t
 */
void mSPIClass::transferBytes(const uint8_t * out, uint8_t * in, uint32_t size) {
    while(size) {
        if(size > 64) {
...
void mSPIClass::transferBytesAligned_(const uint8_t * out, uint8_t * in, uint8_t size) {
    if (!size)
        return;
...
void mSPIClass::transferBytes_(const uint8_t * out, uint8_t * in, uint8_t size) {
