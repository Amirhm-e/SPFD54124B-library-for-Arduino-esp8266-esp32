        ...
        *clkport &= ~clkpinmask;
        ...
        for (uint8_t bit = 0x80; bit; bit >>= 1)
        {
            ...
            GPOC = (1 << sclk);
				...
				#elif defined(ESP32) || defined(ESP32S2)
    // ===== ESP32 / ESP32-S2: 9-bit SPI شبیه‌سازی شده =====
    // بیت 9ام = 0 یعنی Command
    uint16_t data = 0x000 | c;

    // مطمئن شدن از خروجی بودن پین‌ها
    gpio_set_direction((gpio_num_t)sid, GPIO_MODE_OUTPUT);
    gpio_set_direction((gpio_num_t)sclk, GPIO_MODE_OUTPUT);

    for (int i = 8; i >= 0; i--) {
        if ((data >> i) & 1)
            GPIO.out_w1ts = (1 << sid);  // HIGH
        else
            GPIO.out_w1tc = (1 << sid);  // LOW

        GPIO.out_w1ts = (1 << sclk);     // Clock HIGH
        GPIO.out_w1tc = (1 << sclk);     // Clock LOW
    }
#else
...
#if defined(ESP8266)
        mSPI.begin();
        mSPI.setFrequency(160000000L);
#endif

#if defined(ESP32) || defined(ESP32S2)
    SPI.begin(sclk, -1, sid, cs);   // فقط MOSI و SCLK استفاده می‌کنیم
		SPI.setFrequency(200000000L);
#endif
    }
